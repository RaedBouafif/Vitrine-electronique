...
export class WidgetFiltersComponent implements OnInit, OnDestroy {
    ...
    isActiveCategory(){
        if(this.subcategory){
            return 'filter-categories__item--parent'
        }
            else {
                return 'filter-categories__item--current'
            }
        
        
    }
    isActive(item:SubCategory){
        if(this.subcategory){
            if(item._id===this.subcategory._id){
                return 'filter-categories__item--current'
            }
            else {
                return 'filter-categories__item--child'
            }
        }
        else {
            return 'filter-categories__item--child'
    ...
    ngOnInit(): void {
        this.pageCategory.listC$.pipe(
            takeUntil(this.destroy$),
        ).subscribe(data => {
            this.categories=data.categories.filter(item=>item.active===true);
            this.category=data.category
            if(this.category){
                this.category.subCategories=this.category.subCategories.filter(item=>item.active===true);
            }
